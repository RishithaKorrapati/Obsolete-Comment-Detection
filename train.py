...
#     max_memory = {
#         0: "6GiB",        # adjust if your GPU has more VRAM
#         "cpu": "32GiB"
#     }

#     model = AutoModelForCausalLM.from_pretrained(
#         base_model,
#         torch_dtype=torch.float16,   # use fp16 on Windows
...
#     # AdamW optimizer (Windows-friendly; no Triton dependency)
#     optimizer = AdamW(
...
def train(
    ...
    # NOTE: Your GPU is 4GB (RTX 2050). Keep these conservative.
    max_memory = {
        0: "6GiB",   # can be slightly above VRAM; accelerate will offload
        "cpu": "32GiB"
    }
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        mask_input_ids = [-100] * user_prompt_len + tokenized_full_prompt["labels"][user_prompt_len:]
        ...
        idx = min(true_index, false_index)

        tokenized_full_prompt["labels"] = [idx] + tokenized_full_prompt["labels"]
        tokenized_full_prompt["input_ids"] = [idx] + tokenized_full_prompt["input_ids"]
    ...
    # IMPORTANT: make sure we truly have supervised tokens (not all -100).
    # Remember: labels[0] is your "index token", so check labels[1:].
    def has_supervised_tokens(example):
        labels = example.get("labels", [])
        if labels is None or len(labels) <= 1:
            return False
        return any(x != -100 for x in labels[1:])
    ...
    if val_set_size > 0:
        ...
        train_data = train_val["train"].shuffle(2024).map(generate_and_tokenize_prompt)
        train_data = train_data.filter(has_supervised_tokens)

        val_data = train_val["test"].shuffle(2024).map(generate_and_tokenize_prompt)
        val_data = val_data.filter(has_supervised_tokens)
    ...
    # ✅ LIMIT DATASET SIZE (for debugging speed)
    train_data = train_data.select(range(min(len(train_data), 2000)))
    if val_data is not None:
        val_data = val_data.select(range(min(len(val_data), 200)))
    ...
    len_dataset = len(train_data)
    steps_per_epoch = (len_dataset // batch_size) if (len_dataset % batch_size == 0) else (len_dataset // batch_size + 1)
